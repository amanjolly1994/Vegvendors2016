...
function All_mainJson_encoder()
{

	global $db;
	$q3 = "SELECT `place_code` FROM `places`";
					$big_mainjson=new stdClass();
                 $res3 = $db->query($q3);
                //$big_mainjson = array();
                 while( $row3 = $res3->fetch_assoc() )
                 {
                		...
                		$big_mainjson->$row3['place_code']=mainJson($row3['place_code']);
	
                 }

                 return json_encode($big_mainjson);
...
//$placecode = 4;
  ...
  $mahajson->status="changed";
  $mahajson->timestamp=mainJsonFx_timestamp_maker();
  ...
  $mahajson->timeslot_availability=time_slot_fetcher($placecode);
  $mahajson->sabziTimestamp=sabziPrice_timestamp_maker();
  $mahajson->sabzi_status="same";
  $mahajson->vendorTimestamp=sabzi_wala_timestamp_maker();
  $mahajson->vendor_status="same";

		// $mahajson_outer_cover=new stdClass();
		// $mahajson_outer_cover->$placecode=$mahajson;
...
function mainJsonFx_timestamp_maker()
{
	global $db;
	$q3 = "SELECT `update_timestamp` FROM `table_timestamp` where `tableName` = 'MainJson' ORDER BY `tableName` ASC";
...
function sabziPrice_timestamp_maker()
{
	global $db;
	$q3 = "SELECT `update_timestamp` FROM `table_timestamp` where `tableName` = 'sabzi_price' ORDER BY `tableName` ASC";
...
function sabzi_wala_timestamp_maker()
{
	global $db;
	$q3 = "SELECT `update_timestamp` FROM `table_timestamp` where `tableName` = 'sabzi_wala' ORDER BY `tableName` ASC";
...
{		
			global $db;
		$placecode=$placecode;
		$q = "SELECT sp.sabzi_id, sp.price_per_kg,sp.availability, s.sabzi_name,s.sabzi_category ,s.rate FROM `sabzi_price` sp INNER JOIN `sabziz` s ON sp.sabzi_id = s.sabzi_id WHERE sp.place_code = $placecode ORDER BY s.sabzi_category";
		...
		while( $row = $res->fetch_assoc() )
		{
			...
			if( $same == $row['sabzi_category'] )
			{
				...
				$subdata->availability=$row['availability'];
		...
		$sabzi_list->sabziTimestamp=sabziPrice_timestamp_maker();
		$sabzi_list->vendorTimestamp=sabzi_wala_timestamp_maker();
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
function time_slot_fetcher($placecode)
{
global $db;
$response = array();
	...
	if ($res=$db->query("SELECT timeslot_id,availability FROM timeslot_availability where place_code='$placecode'")) 
	{
		 	while( $row = $res->fetch_assoc())
	       {
