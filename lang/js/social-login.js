...
function onSuccess(googleUser) {
      ...
      console.log("Google Login By "+profile.getName());
      var profile = googleUser.getBasicProfile();
  ...
  console.log("Google Login: "+name);
  facebookSave(id,name,email,gender,picUrl,"google");
  }
  ...
  // Google JavaScript SDK ends

  // Facebook JavaScript SDK

  //Initiate and Setup Facebook js sdk v2.6
	window.fbAsyncInit = function() {
	    FB.init({
	      appId      : '572841012892700',
	      xfbml      : true,
	      version    : 'v2.6'
	    });
	};
	...
	function getInfo() {
		//alert("in getInfo function");
		FB.api('/me', 'GET', {fields: 'name,id,email,gender,locale,picture'}, function(response) {
			var id = response.id;
			var name = response.name;
			var email = response.email;
			var gender = response.gender;
			var picUrl = response.picture.data.url;
			facebookSave(id,name,email,gender,picUrl,"facebook");
			/*document.getElementById('status').innerHTML = response.id;
    document.getElementById('name').innerHTML = response.name+" "+response.email+" "+response.gender+" "+response.locale;
  ...
  // Common G+F SDK DB entry via AJAX

  function facebookSave(id,name,email,gender,picUrl,oauth)
	{
		//alert("In facebookSave function");
		//var seta = 'id='+id+'&name='+name+'&email='+email+'&gender='+gender+'&picUrl='+picUrl;

		$.ajax({

			type : 'POST',
			url : '/lang/ajax/facebook_login.php',
			data : {
				id : id,
				name : name,
				email : email,
			...
			success : function(value)
			{
				if( value == "done" )
					...
					console.log("not saved in facebook");
			}
  ...
  {
  		$(parent.document).find(".messageBox").fadeIn();

  		if( $(parent.document).find(".messageBoxWrapper").length ){
  			//exists.
  		}
  		else{
  			var messageBoxWrapper = '<div class="messageBoxWrapper"></div>';
  			$(parent.document).find(".common").append(messageBoxWrapper);
  		}

  		if( $(parent.document).find(".messageBox").length ){
  			$(parent.document).find(".messageBox").css({background: type});
  			$(parent.document).find(".messageBox .mls").text(messageText);
  		}
  		else{
  			var messageBox = '<div class="messageBox closeBox"><span class="mls"></span></div>';
  			$(parent.document).find(".messageBoxWrapper").append(messageBox);
  		...
  		$(parent.document).find(".closeBox").click(function(){
  			$(this).fadeOut();
  		});

  		$(parent.document).find(".messageBox").delay(2000).fadeOut();
